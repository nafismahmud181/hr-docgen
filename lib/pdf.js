...
import { buildDocument, buildRefNumber, formatDate } from "./templates";
...
export async function generatePdf(typeKey, employee, options = {}) {
  const base = buildDocument(typeKey, employee, options);
  ...
  const doc = options.override
    ...
    : base;
  const refNo = buildRefNumber(typeKey, employee);
  ...
  y -= 56; // space for a handwritten signature
  page.drawLine({
