...
// ── Shared documents ───────────────────────────────────────────────────
// A generated PDF snapshot stored under an unguessable id, downloadable via a
// public link. The PDF bytes are stored privately and streamed through our
// own route; only metadata lives in shares.json.

const SHARES = "shares.json";
const shareBlobName = (id) => `share-${id}.pdf`;

export async function createShare(meta, bytes) {
  const shares = await readJson(SHARES, []);
  const share = { id: crypto.randomUUID(), createdAt: new Date().toISOString(), ...meta };
  await writeBinary(shareBlobName(share.id), bytes, "application/pdf");
  shares.push(share);
  await writeJson(SHARES, shares);
  return share;
}
...
export async function getSharePdf(id) {
  return readBinary(shareBlobName(id));
}

export async function getSharesForEmployee(employeeId) {
  const shares = await readJson(SHARES, []);
  return shares
...
export async function deleteShare(id) {
  ...
  await deleteBinary(shareBlobName(id));
  await writeJson(SHARES, next);
  return next.length !== shares.length;
}
