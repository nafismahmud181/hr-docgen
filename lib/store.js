...
// Blob is available either via a read-write token, or (the modern default)
// via an OIDC connection, which injects BLOB_STORE_ID and a VERCEL_OIDC_TOKEN
// at runtime. The @vercel/blob SDK picks up whichever is present automatically.
const useBlob = Boolean(process.env.BLOB_READ_WRITE_TOKEN || process.env.BLOB_STORE_ID);
const DATA_DIR = path.join(process.cwd(), "data");

export function storageMode() {
  return useBlob ? "blob" : "file";
}

// On Vercel the filesystem is read-only, so a Blob store (token) is required.
// Locally the filesystem is writable, so files are fine.
export function isPersistentStorageReady() {
  if (process.env.VERCEL) return useBlob;
  return true;
}
